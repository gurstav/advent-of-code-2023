...
class Map:
    ...
    def _get_min_destination_value(self, source_range, range):
        ''' The source_destination_range is monotonically increasing with input seed which allows conclusions to be drawn about destination values

        Fully inside:
        Source range                |--------------->|
        SD Range                |--------------------->|

        Beginning of source range inside range:
        Source range                        |--------------->|
        SD Range                |--------------------->|

        End of source range inside range:
        Source range    |--------------->|
        SD Range                |--------------------->|

        Fully outside, there would be no mapping and the min seed is the start of the source range.

        '''
        # Source range fully inside. Min destination value is destination value for source range start.
        if source_range["start"] >= range["source_range_start"] and source_range["end"] <= range["source_range_end"]:
            return self.apply(source_range["start"])
        # Beginning of source range inside. Min destination value is destination value for source range start.
        elif range["source_range_start"] <= source_range["start"] and range["source_range_end"] <= source_range["end"]:
            return self.apply(source_range["start"])
        # End of source range inside. Min destination value is destination range start.
        elif range["source_range_start"] >= source_range["start"] and range["source_range_end"] >= source_range["end"]:
            return range["destination_range_start"]
        
        return source_range["start"]
    
    def apply_to_range(self, source_range):
        ''' For each source destination range, find the minimum destination value for the whole source range, instead of one per each value '''
        for range in self.source_destination_ranges:
            return self._get_min_destination_value(source_range, range)
...
def get_seed_ranges(content):
    seeds_content = content["seeds"].copy()
    seeds_content.reverse()
    seed_ranges = []
    while (seeds_content):
        seed_range = {}
        start = int(seeds_content.pop())
        seed_range["start"] = start
        length = int(seeds_content.pop())
...
if __name__ == '__main__':
    ...
    seed_numbers = set(content["seeds"])
    seed_ranges = get_seed_ranges(content)
    min_location = 999999999999999999999999999999999999
    for seed_range in seed_ranges:
        print(seed_range)
        for seed in range(seed_range["start"], seed_range["end"]):
            current_location = get_location(seed)
            if current_location < min_location:
                min_location = current_location
    print(min_location)


    # 340994526 wrong (too high)